...
**Stable release: v0.48.0 - channel=stable - Action Schema 23 - Engineering API 5** - adds **Automated Engineering Workflows I**. The new `stm32.deep_diagnostics` workflow collapses a common debug triage chain into one typed operation: start a constrained debug session with RTOS auto-awareness, halt, capture registers and Cortex-M fault state, collect best-effort exception-frame/RTOS/disassembly evidence, capture stack frames, classify evidence quality, and always release the probe. Optional evidence may degrade without hiding the primary fault/stack result. No arbitrary shell/GDB/Tcl input, memory/register write, TCB-layout guessing or widened permission surface is introduced.
