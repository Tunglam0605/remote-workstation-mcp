...
**Stable release: v0.27.1 ? channel=stable ? Action Schema 11 ? Engineering API 5** ? adds the owner-enabled `antigravity-local` worker provider backed by Google's official Antigravity CLI (`agy`). RWMCP uses documented headless JSON/stream-json interfaces, sends task prompts over stdin, enforces `--sandbox`, never passes `--dangerously-skip-permissions`, never reads Google OAuth/keyring credentials, and exposes safe model/quota inspection through `antigravity_status`. The v0.26 Control Center redesign and v0.25 Codex Cockpit-pool integration remain intact. v0.27.1 hardens dispatch by treating explicit headless permission auto-denials as blocked work even if `agy` reports a nominal `SUCCESS` envelope.
