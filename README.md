...
**Stable release: v0.35.0 - channel=stable - Action Schema 14 - Engineering API 5** - adds owner-managed **Smart Routing** on top of the existing Agent Control policy without creating a second authority path. The owner can select `Direct`, `Codex assisted`, `Smart routing`, or `Custom`; these are presets over the existing low-level execution policy. The new read-only `worker_route_plan` action combines task intent, effective Work Session policy, provider readiness, Codex budgets and fallback to recommend RWMCP, Codex, Antigravity or stop, but never dispatches work or expands permissions. Smart routing keeps workstation/read operations on RWMCP, prefers Codex for bounded coding/review, and uses sandboxed Antigravity as the frontend/UI specialist with Codex/RWMCP fallback. v0.35.0 retains the v0.34.1 Windows tunnel-client repair, Windows-only Office Word capability pack, Codex account-pool routing and all existing Engineering capabilities.
