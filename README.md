...
**Stable release: v0.37.0 - channel=stable - Action Schema 15 - Engineering API 5** - reorganizes Moonlight Control Center around nine long-lived user-intent domains: Overview, Work, Agents, Engineering, Office, Web, Devices, Security and System. Provider and implementation details such as Codex, Antigravity, Playwright, Existing Chrome, NotebookLM, OOXML and COM stay below the domain layer instead of becoming top-level navigation. Work, Engineering, Office and Web discover their capability groups from the running backend; legacy `#execution`, `#access`, `#updates` and `#settings` deep links remain compatible and open their original detailed views while highlighting the new canonical domain. v0.37.0 is an owner-local Control Center information-architecture/UX release: it preserves the v0.36 Web Automation and NotebookLM Video Studio capabilities, the owner authority model, Action Schema 15 and Engineering API 5.
