...
**Stable release: v0.25.0 ? channel=stable ? Action Schema 10 ? Engineering API 5** ? adds the Codex Account Broker: RWMCP can keep the existing native Codex login or, when the owner explicitly selects it and Cockpit Tools reports a healthy loopback API account pool, route bounded Codex workers through Cockpit auto-routing/session-affinity/cooling without decrypting Cockpit credentials or rewriting `~/.codex/auth.json`. Pool API keys stay child-process-only; an unhealthy/disabled/empty pool fails closed and existing Codex limit policy can still fall back to `rwmcp-only`. v0.24.1 Ubuntu typed reboot and v0.24.0 STM32 `.ioc` engineering depth remain intact.
