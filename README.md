...
**Stable release: v0.38.0 - channel=stable - Action Schema 15 - Engineering API 5** - upgrades AI worker orchestration so Codex and Antigravity are both general-purpose bounded workers with task affinity rather than hard role locks. Smart routing prefers Antigravity for frontend/UI and Codex for coding/backend/engineering, can cross-fallback to the alternate AI worker only for quota/rate-limit/auth/provider-availability failures, and falls back the affected Work Session to direct RWMCP only after all allowed AI workers are exhausted. Ordinary implementation/build/test failures do not silently change workers. Task Attempts now persist a bounded provider-attempt trace, and the Moonlight Agents page exposes task affinity, fallback chains, worker pool/capacity, live readiness, and final RWMCP fallback. v0.38.0 preserves the v0.37 nine-domain Control Center structure and keeps Action Schema 15 / Engineering API 5.
