...
**Stable release: v0.40.1 - channel=stable - Action Schema 16 - Engineering API 5** - refines the v0.40 Control Center UX with a rebalanced **Security & access** page while preserving all v0.39/v0.40 routing, authority, and backend contracts. The Security page now uses a compact header, three high-signal status summaries, balanced Permission mode and Full-control lease cards, a full-width collapsed Advanced access scopes block, and a full-width Admin requests section with clearer command/reason hierarchy and collapsed command hashes. The patch also removes visible mojibake in Access view helper/status copy. No MCP tool schema or Engineering API changes are introduced.
