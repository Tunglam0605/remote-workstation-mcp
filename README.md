...
**Stable release: v0.44.1 - channel=stable - Action Schema 21 - Engineering API 5** - hardens the v0.44 Professional Tooling Expansion against upstream vendor contracts before further tool growth. OpenOCD STM32 deploy no longer performs a redundant second `verify_image` after `program ... verify`; explicit `firmware.verify` remains available for independent re-verification. ROS 2 topic Hz/BW results are explicitly receiver-side observations, and `ros2_topic_hz` now exposes the upstream `--wall-time` option for simulation/ROS-time stalls. KiCad provider status probes optional CLI capabilities so board statistics fail clearly or degrade gracefully on older KiCad releases instead of surfacing opaque command errors. Word, Excel and PowerPoint native acceptance now narrows `AutomationSecurity=ForceDisable` to the programmatic-open window and restores the previous Office security setting afterward. The full vendor audit and follow-up backlog are recorded in `docs/PROFESSIONAL_TOOLING_VENDOR_AUDIT.md`.
