...
**Stable release: v0.44.1 - channel=stable - Action Schema 20 - Engineering API 5** - is a **Vendor Reference Hardening** patch for the v0.44 Professional Tooling Expansion. No new professional domain or MCP action is added. OpenOCD flash/deploy now uses the upstream single `program ... verify reset exit` flow instead of duplicate verification; GDB watchpoints no longer overclaim hardware guarantees; ROS 2 Hz/BW results explicitly identify subscriber-observed semantics; KiCad fabrication validation requests board/schematic parity and blocks active unconnected/parity findings; Excel scans and blocks external/side-effect formulas before native acceptance while allowing ordinary structured references; Excel and PowerPoint restore `AutomationSecurity` immediately after programmatic Open; and PowerPoint text edits preserve simple paragraph/run formatting in place while failing closed on mixed formatting, hyperlinks, fields, breaks or paragraph reshaping. The detailed vendor/upstream audit is in `docs/audits/PROFESSIONAL_TOOLING_VENDOR_AUDIT.md`.
