...
**Stable release: v0.32.3 — channel=stable — Action Schema 11 — Engineering API 5** — adds hardened delegated-worker execution for Codex and Antigravity while keeping ChatGPT Web and owner-local policy authoritative. Codex can route through the loopback-only Cockpit API pool, pin an owner-selected model, and project only validated Engineering Agent Stack (EAS) roles into an isolated invocation while user config/plugins remain ignored. Antigravity headless dispatch requires terminal sandboxing with `proceed-in-sandbox` and fails closed otherwise. Moonlight Execution exposes Codex model/EAS controls alongside the existing provider, fallback, budget, and broker settings. v0.32.3 also polishes the non-Overview Moonlight pages (Access, Execution, Devices, Updates, Settings) with more balanced grids, section hierarchy, responsive spacing, and consistent action treatments while leaving the Overview artwork composition untouched. No arbitrary shell, credential extraction, permission bypass, or autonomous authority is added.
