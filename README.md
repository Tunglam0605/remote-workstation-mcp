...
**Stable release: v0.51.0 - channel=stable - Action Schema 24 - Engineering API 5** - adds **NotebookLM Command Automation II**. `notebooklm_video_generate` now accepts either one focus prompt or a bounded sequential batch of up to 25 video jobs, reuses one authenticated Existing Chrome claim, verifies STARTED/READY postconditions per job, and stops fail-closed on the first error by default. `notebooklm_ask` can also auto-claim/release an authenticated notebook tab for a single command. These workflows use semantic find/fill/click operations rather than coordinate mouse/screen driving and do not expose Google credentials, cookies, tokens, raw selectors or JavaScript.
