...
**Stable release: v0.34.0 - channel=stable - Action Schema 13 - Engineering API 5** - redesigns the Moonlight Execution page as **Agent Control** without expanding backend authority. The Control Center now separates the global route, default strategy (`RWMCP only`, `Codex first`, `Hybrid`), Codex and Antigravity worker readiness, Work Session override policy, Codex-limit fallback, and advanced model/budget/account-broker controls. Disabled and unavailable workers are distinct, session-level routing is not misrepresented as a global fact, recovery actions are separated from normal Save, and the page is responsive/localized for desktop and mobile. Codex and Antigravity remain bounded implementation workers behind isolated Work Sessions, sandboxing and owner policy; `--dangerously-skip-permissions` remains prohibited. v0.34.0 retains the Windows-only Office Word capability pack from v0.33.2, shared Codex skills isolation, EAS hardening, Antigravity sandbox policy, and all existing Engineering capabilities.
