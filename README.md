...
**Stable release: v0.30.0 — channel=stable — Action Schema 11 — Engineering API 5** — refines the seasonal/event Control Center into the approved premium engineering-cockpit visual direction: full-width cinematic hero, richer theme-specific lighting, Vietnamese heritage motifs, Dong Son-inspired ornament, layered skyline, improved countdown/status glass cards, responsive polish, and reduced-motion safety. v0.29.0 introduced the production theme resolver, four-season fallback, Vietnamese event packs, manual override, motion controls, and timezone-aware event scheduling through 2030; backend/API/security authority remains unchanged.
