...
**Stable release: v0.29.0 ? channel=stable ? Action Schema 11 ? Engineering API 5** ? adds a production seasonal/event presentation system to the Control Center without changing MCP authority or backend/API/security contracts. The owner can use event-first, seasonal-only, or manual themes; motion and decoration density are configurable; Vietnamese event packs include Tết Nguyên Đán, Trung Thu, Giỗ Tổ Hùng Vương, 30/4, 1/5, 2/9 and additional cultural days, with four-season fallback. The current Vietnam event resolver is timezone-aware, covers lunar-event dates through 2030, and falls back to Spring/Summer/Autumn/Winter outside event windows. v0.28.0 Control Center refinement, v0.27.3 Antigravity headless-worktree hardening, and v0.25 Codex Cockpit-pool integration remain intact.
