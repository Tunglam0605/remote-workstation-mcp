...
**Stable release: v0.39.0 - channel=stable - Action Schema 16 - Engineering API 5** - introduces the **Unified Three-Target Orchestrator**. `rwmcp-direct`, `codex-local`, and `antigravity-local` are peer execution targets at the routing layer while preserving their distinct deterministic, sandbox, permission, and quota boundaries. Owner policy exposes eight target sets (Auto/Smart, each single target, all three pairs, and All three), and a bounded Work Session may apply the same target-set model through `execution_target_set_override` when chat overrides are allowed. Task affinity prefers Antigravity for frontend/UI, Codex for code/backend/engineering, and RWMCP for read/workstation/Office/build/test work without hard capability locks. Clean isolated work may safely hand off to the next allowed target; dirty worktrees fail closed for review, and direct RWMCP handoff is recorded honestly rather than reported as fake worker success. Moonlight Control Center exposes the target set, affinity chain, provider readiness, and safety ceiling without widening workstation authority.
