...
**Stable release: v0.31.0 — channel=stable — Action Schema 11 — Engineering API 5** — upgrades the seasonal/event Control Center to local artwork-backed theme packs built from the approved visual references. Spring, Summer, Autumn, Winter, Tết, Mid-Autumn, Hùng Kings, 30/4, 1/5 and 2/9 now ship with offline hero/rail WebP assets, glass-card integration and live countdown/status overlays while preserving backend/API/security authority. v0.30.0 remains the CSS/SVG visual foundation and v0.29.0 remains the theme resolver/event-calendar foundation.
