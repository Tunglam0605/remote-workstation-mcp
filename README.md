...
**Stable release: v0.44.0 - channel=stable - Action Schema 20 - Engineering API 5** - introduces the **Professional Tooling Expansion**. STM32/embedded diagnostics gain typed ELF/AXF memory reports, halted-frame locals, bounded disassembly and safe hardware watchpoints without arbitrary GDB or memory-write surfaces. ROS 2 gains node/action introspection, bounded topic rate/bandwidth sampling, TF2 lookup and lifecycle inspection/allowlisted transitions; action-goal dispatch remains intentionally deferred until durable goal-handle cancellation is available. KiCad gains direct provider/board-stat/DRC/ERC/validation/BOM reports using temporary outputs and no arbitrary plugins. The Windows Office Capability Pack now has transactional Excel and PowerPoint support alongside Word: Excel provides typed cell/range/formula edits with recalculation, formula-error and PDF acceptance; PowerPoint provides bounded existing title/shape text edits with structure-invariant validation and native read-only open/PDF acceptance. Both use Work Session ownership, file leases, SHA-256 conflict detection, backup/rollback, active-content blocking and no raw COM/VBA surface.
