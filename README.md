...
**Stable release: v0.34.0 - channel=stable - Action Schema 14 - Engineering API 5** - adds a simpler, owner-controlled AI worker routing layer for ChatGPT Web. The Moonlight Execution page now leads with four routing profiles (`Direct`, `Codex assisted`, `Smart routing`, `Custom`), live provider-readiness cards, explicit chat override/fallback controls, and collapsed advanced provider settings. The new read-only `worker_route_plan` action lets ChatGPT combine task intent, owner routing profile, effective Work Session policy, and live Codex/Antigravity readiness before delegation; it never dispatches work or relaxes permissions. `Smart routing` prefers Codex for general engineering code/review and Antigravity for frontend/UI work, with RWMCP as the bounded fallback. Antigravity dispatch prompts are additionally hardened to forbid privilege escalation, admin/root access, owner-policy changes, and work outside the assigned sandboxed worktree. v0.34.0 retains the Windows-only Office Word capability pack from v0.33.2, shared Codex skills isolation, EAS worker hardening, Cockpit account-pool routing, and all existing Engineering capabilities.
