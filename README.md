...
**Stable release: v0.45.1 - channel=stable - Action Schema 21 - Engineering API 5** - is the **Trust & Reliability Hardening** patch for v0.45.0. It records a full capability-family trust audit against vendor/upstream contracts, moves Git status to porcelain v2 for machine parsing, capability-probes optional KiCad board statistics, exposes OpenOCD build provenance, marks unresolved CMSIS-SVD `derivedFrom` inheritance explicitly, normalizes degraded systemd journal collection, rejects suspicious empty Antigravity success, and adds an explicit dependency install-script allowlist enforced by CI/prepack. No new public authority is introduced; existing Work Session, resource lease, Office transaction, cross-node default-deny and debugger/firmware safety boundaries remain unchanged.
