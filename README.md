...
**Stable release: v0.32.4 — channel=stable — Action Schema 11 — Engineering API 5** — adds hardened delegated-worker execution for Codex and Antigravity while keeping ChatGPT Web and owner-local policy authoritative. Codex can route through the loopback-only Cockpit API pool, pin an owner-selected model, and project only validated Engineering Agent Stack (EAS) roles into an isolated invocation. v0.32.4 adds owner-controlled shared Codex skills: VSCode and standalone Codex keep one canonical `CODEX_HOME`, while RWMCP workers receive a bounded worktree-local snapshot of the same `skills/` tree without inheriting the owner's Codex config, plugins, MCP servers, browser/computer-use settings, or credentials. Antigravity headless dispatch requires terminal sandboxing with `proceed-in-sandbox` and fails closed otherwise. Moonlight Execution exposes Codex model, EAS and shared-skills controls alongside the existing provider, fallback, budget, and broker settings. The non-Overview Moonlight pages retain the v0.32.3 balanced grids, section hierarchy, responsive spacing, and action treatments while leaving the Overview artwork composition untouched. No arbitrary shell, credential extraction, permission bypass, or autonomous authority is added.
