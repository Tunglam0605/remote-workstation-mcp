...
**Stable release: v0.42.0 - channel=stable - Action Schema 18 - Engineering API 5** - introduces the **Generic Three-Target Execution Policy**. `rwmcp-direct`, `codex-local`, and `antigravity-local` now share one canonical `targetPolicy` for enabled targets, terminal fallback and per-target task budgets. Codex and Antigravity budgets are enforced symmetrically by the same dispatch guard, while Work Session overrides, fallback latches, worktree ownership and permission boundaries can only narrow authority. v0.41 Codex-centric settings/counters remain as a dual-read/dual-write compatibility projection so older clients and rollback remain safe. The Control Center keeps common routing choices simple and places generic per-provider budgets under Technical details.
