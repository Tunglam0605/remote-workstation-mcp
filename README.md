...
**Stable release: v0.43.0 - channel=stable - Action Schema 19 - Engineering API 5** - introduces **Objective Decomposition + Bounded Multi-Agent Waves**. ChatGPT can supply one bounded plan using local task keys and dependencies; RWMCP validates and persists the complete DAG atomically, resolves keys to durable task IDs, and deterministically assigns `auto-worker` tasks from task affinity plus the effective owner/Work Session target policy. Frontend/UI prefers Antigravity, coding/debug/review prefers Codex, and deterministic typed engineering/workstation work remains RWMCP. `work_objective_execute_wave` executes only one scheduler-ready wave per call: AI workers sharing the Work Session worktree are serialized, while compatible deterministic tasks may run in parallel. RWMCP still does not embed a second planning model, does not loop an objective to completion, and does not widen Work Session, provider, permission, lease, or interlock authority.
