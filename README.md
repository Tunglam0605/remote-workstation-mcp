...
**Stable release: v0.45.0 - channel=stable - Action Schema 21 - Engineering API 5** - advances the **Professional Tools Expansion** on top of the v0.44.1 vendor-hardening baseline. GDB/MI result handling now uses one bounded recursive parser instead of scattered regex extraction; ROS 2 topic-rate, bandwidth and TF diagnostics gain an optional typed `rclpy` JSON helper when an explicit distro is configured, while retaining the bounded CLI fallback; and STM32 adds project-scoped read-only CMSIS-SVD inspection for device, CPU, peripheral, register, cluster, array and field metadata. The new SVD surface rejects DTD/entities, is size/count bounded, never connects to target hardware and exposes no register-write operation. Existing Work Session ownership, resource leases, OpenOCD/GDB authority boundaries and Office/KiCad transaction semantics remain unchanged.
