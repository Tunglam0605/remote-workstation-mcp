...
**Stable release: v0.36.0 - channel=stable - Action Schema 15 - Engineering API 5** - adds a bounded Web Automation platform and authenticated NotebookLM integration without changing the owner-authority model. Managed Chrome/Edge provides semantic navigation/interaction, typed waits, controlled upload/download artifacts and dedicated persistent profiles. For Google-authenticated NotebookLM, RWMCP uses an owner-installed Manifest V3 Existing Chrome Bridge with exact-origin Native Messaging and local authenticated IPC instead of automating or bypassing Google sign-in. The NotebookLM adapter adds authenticated notebook status/source inventory, ask-and-wait, and deterministic Video Overview generation whose success requires STARTED evidence followed by disappearance of the generation-busy marker and appearance of a new READY video artifact. Browser sessions and Existing Chrome claims remain principal + Work Session owned; password fields, cookies/tokens, raw JavaScript/selectors/CDP, coordinate clicks and LAN debugging endpoints remain unavailable. v0.36.0 retains v0.35 Smart Routing, the Windows-only Office Word pack, Codex/Antigravity worker controls and all existing Engineering capabilities.
