...
**Stable release: v0.31.0 — channel=stable — Action Schema 11 — Engineering API 5** — introduces local artwork-backed seasonal presentation. The Mid-Autumn theme now uses bundled WebP hero/backdrop artwork with glassmorphism overlays, stronger page ambiance, offline asset serving and no remote dependencies; all existing backend/API/security authority remains unchanged. v0.30.0 remains the CSS/SVG theme-engine baseline.
