...
**Stable release: v0.33.2 - channel=stable - Action Schema 13 - Engineering API 5** - introduces the first production RWMCP Office Capability Pack as a sibling of Engineering. The initial Word surface is deliberately compact: `office_capabilities`, `word_inspect`, and transactional `word_edit`. Word inspection is OOXML-based and returns structural headings, paragraphs, tables, images, equations, sections, security findings, and semantic locators. Mutations require an explicit Work Session, a per-file exclusive lease, backup and working copy, SHA-256 optimistic concurrency, typed bounded operations, structural acceptance, and rollback records. On Windows, native Microsoft Word COM acceptance can reopen the working copy, verify OMath equations, export PDF evidence, and clean up the RWMCP-owned Word process. Macro-enabled mutation, ActiveX, embedded OLE, remote templates, arbitrary VBA/shell/COM calls, and implicit UI automation remain blocked. The Office MCP surface is Windows-only; Linux/macOS nodes register no Office tools or Office capabilities and continue running the rest of RWMCP without Microsoft Office dependencies. v0.33.2 also retains the v0.32.4 shared Codex skills isolation, Codex/EAS worker hardening, Antigravity sandbox policy, and all existing Engineering capabilities.
