...
**Stable release: v0.38.1 - channel=stable - Action Schema 15 - Engineering API 5** - hardens Windows headless Antigravity delegation so the bounded worker stays on workspace file read/write operations instead of invoking terminal/command tools that can trigger non-interactive permission escalation. Command-based build, test and Git verification remain owned by ChatGPT/RWMCP on Windows; the provider still runs inside the configured sandbox, keeps permission failures fail-closed, and does not use `--dangerously-skip-permissions`. v0.38.1 retains symmetric Codex/Antigravity capacity fallback, the nine-domain Moonlight Control Center and all v0.38 authority contracts.
