...
**Stable release: v0.47.0 - channel=stable - Action Schema 23 - Engineering API 5** - adds **Cortex-M + RTOS read-only professional diagnostics**. Debug sessions can opt into bounded OpenOCD RTOS awareness (`none | auto | freertos`), `debug_rtos_tasks` consumes the documented GDB/MI `-thread-info` contract without guessing FreeRTOS TCB layouts, and `debug_cortexm_exception_frame` decodes the architectural stacked Cortex-M exception frame from EXC_RETURN-selected MSP/PSP context. Fault snapshots now expose CMSIS-Core SCB provenance and raw bounded SCB register evidence. No arbitrary GDB/Tcl command, register write, memory write, TCB-offset parser or GDB flash authority is added.
