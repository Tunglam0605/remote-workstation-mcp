...
**Stable release: v0.26.0 ? channel=stable ? Action Schema 10 ? Engineering API 5** ? redesigns the owner-local Web Control Center into a compact Cockpit-inspired application shell: persistent sidebar navigation, page-scoped content, cleaner status cards, a dedicated Execution & Codex surface, a separate Updates page, responsive mobile navigation, and complete Vietnamese/English labels for dynamic execution state. Existing element IDs, backend APIs, owner policy and security boundaries remain unchanged. v0.25.3 Codex Cockpit-pool dispatch hardening remains intact.
