...
**Stable release: v0.40.0 - channel=stable - Action Schema 16 - Engineering API 5** - delivers **Control Center UX Simplification** on top of the v0.39 three-target architecture without changing execution authority or tool schemas. Moonlight now groups the stable routes into Home / Work / AI / Tools / System, leads Overview with workstation health, AI routing, attention state and quick actions, recommends Auto / Smart in a simplified AI routing page, keeps all eight target sets behind progressive disclosure, presents Work / Engineering / Office / Web as friendly task domains, and collapses advanced security, pairing, multi-node, broker, budget and recovery controls until needed. Vietnamese-first copy is retained with English fallback; all existing backend routes, security boundaries, routing affinity, provider sandboxing and owner controls remain intact.
