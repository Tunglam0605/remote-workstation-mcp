...
**Stable release: v0.28.0 ? channel=stable ? Action Schema 11 ? Engineering API 5** ? adds the owner-enabled `antigravity-local` worker provider backed by Google's official Antigravity CLI (`agy`). RWMCP uses documented headless JSON/stream-json interfaces, sends task prompts over stdin, enforces `--sandbox`, never passes `--dangerously-skip-permissions`, never reads Google OAuth/keyring credentials, and exposes safe model/quota inspection through `antigravity_status`. v0.28.0 refines the Control Center into a denser Cockpit-inspired engineering console with clearer execution/Codex/Antigravity status, improved responsive behavior, and more consistent light/dark visual hierarchy. The v0.27.3 Antigravity headless-worktree hardening and v0.25 Codex Cockpit-pool integration remain intact.
