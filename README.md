...
**Stable release: v0.37.1 - channel=stable - Action Schema 15 - Engineering API 5** - hardens the Windows managed updater for non-interactive RWMCP shells where `ProgramFiles`/`ProgramFiles(x86)` may be absent from the process environment. The release installer now resolves Windows Program Files roots through OS Special Folders before constructing Node/npm/Git candidate paths, preventing the update path from failing before package download/activation. v0.37.1 retains the nine-domain Moonlight information architecture from v0.37.0 and all v0.36 capabilities without changing MCP authority, Action Schema or Engineering API.
