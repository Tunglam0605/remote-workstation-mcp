...
**Stable release: v0.34.1 - channel=stable - Action Schema 13 - Engineering API 5** - Windows installer hotfix: an existing release slot is now revalidated and the pinned OpenAI tunnel-client is repaired before that slot can be activated. This closes the partial-install case where v0.34.0 could be selected while `runtime/openai-tunnel/tunnel-client.exe` was missing. v0.34.1 otherwise retains the v0.34 Agent Control UX, bounded Codex/Antigravity worker model, Windows-only Office Word capability pack, Action Schema 13 and Engineering API 5.
