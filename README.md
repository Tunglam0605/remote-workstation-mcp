...
**Stable release: v0.46.0 - channel=stable - Action Schema 22 - Engineering API 5** - adds **NotebookLM command-mode + Windows recovery hardening**. `notebooklm_video_generate` can now auto-claim an already-authenticated NotebookLM tab, run semantic background commands, verify STARTED/READY postconditions, and release the claim without coordinate mouse/screen driving. Windows direct OpenAI startup now resolves the owner-shared config first, and restart handoff refuses incomplete OpenAI slots that lack `tunnel-client.exe`. Existing Work Session, resource lease, Office transaction, cross-node default-deny and debugger/firmware safety boundaries remain unchanged.
