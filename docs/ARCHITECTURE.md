...
Release identity remains explicit. v0.19.0 reports `serverVersion=0.19.0`, `channel=stable`, optional `gitCommit`, Action Schema 5 and Engineering API 4. v0.19 adds persisted worker-provider task bindings and controlled dispatch through the existing orchestration boundary; production nodes remain independently managed through checksum, health, readiness and rollback gates.

Phase 3 closes without an autonomous provider path. The post-Phase-3 v0.17 foundation introduced two coordination primitives before execution was allowed:
...
- `WorkerProviderRegistry` remains runtime/extension-owned and bounded. MCP may observe descriptor/status/dispatch capability, but provider registration is not an MCP action and provider identity is never an authorization credential.

v0.18 hardens the coordination/security substrate. Project Session Group views continuously revalidate member project identity and report drift/degraded membership instead of trusting join-time metadata forever. Separately, cross-node directional grants gain an owner-local Control Center manager. That manager edits local policy and the dedicated transport scope but is not exposed through MCP; transfer execution still requires matching grant contracts on both participating Direct Nodes.

v0.19 adds controlled provider dispatch without introducing a second executor. A Work Task may persist a `worker-provider` binding containing only `providerId`. `work_objective_execute_task` still accepts identifiers only and routes both engineering-workflow and worker-provider bindings through deterministic Scheduler Awareness, `TaskExecutionCoordinator`, resource leases/node interlocks and one durable Task Attempt per generation. Unregistered/offline providers stay `WAITING_PROVIDER`; worktree-bound providers stay `WAITING_SESSION` until the caller-owned Work Session has an isolated worktree. Provider blocked/failed results fail the task, restart reconciliation marks incomplete attempts interrupted, and no automatic replay occurs.

v0.20 development adds a **human-managed multi-chat coordination layer** without creating a second reasoning system. `ProjectCoordinationService` reads caller-owned same-project Work Sessions and reports role/current-task labels, worktree state, dirty/stale sessions, shared-worktree conflicts and duplicate-task-label overlap signals. `work_session_resume` remains read-only but includes the same-project snapshot for one-call handoff. `work_session_lifecycle_preview` reports mechanical close/cleanup blockers without mutation. `currentTask` is an explicit controller-managed label that may be cleared with `null`; it is not an execution binding or authority token. ChatGPT Web or another authorized client remains responsible for planning, task assignment, architecture choices and review.
