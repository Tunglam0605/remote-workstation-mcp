...
v0.13.x keeps `actionSchemaVersion=2` and `engineeringApiVersion=3`, so an app already refreshed for v0.12 does **not** need another action refresh. The new `stm32.deploy_accept` workflow is discovered through the existing `engineering_workflow_*` actions. v0.13.1 is a Windows recovery-plane hotfix only and does not change the action catalog.
