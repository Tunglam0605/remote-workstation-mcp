...
v0.13.x keeps `actionSchemaVersion=2` and `engineeringApiVersion=3`, so an app already refreshed for v0.12 does **not** need another action refresh. The `stm32.deploy_accept` workflow is discovered through the existing `engineering_workflow_*` actions. v0.13.1 changes only Windows recovery-plane lifecycle behavior; v0.13.2 changes only OpenOCD provider discovery/script resolution; v0.13.3 adds active ST-Link access preflight and `probe-busy` diagnostics inside the existing workflow contract; v0.13.4 only increases adapter-preflight OpenOCD diagnostic verbosity so real xPack ownership failures classify correctly. None of these changes modifies the ChatGPT action catalog.
