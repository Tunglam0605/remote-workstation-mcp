...
v0.22.0-dev.0 continues the hands-only expansion: official JSON/bounded diagnostics for ESP-IDF and PlatformIO, ROS 2 test/bag inspection, Docker container evidence, plus an optional local Codex CLI implementation worker. The Codex provider is runtime opt-in, requires isolated Work Session worktrees, remains absent by default and never becomes a planning/authority source.
