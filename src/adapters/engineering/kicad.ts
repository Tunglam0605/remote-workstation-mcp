...
export class KicadAdapter {
  ...
  async boardStats(workspace: string, projectPath: string, board: string) {
    ...
    if (!await this.commandSupported(cli.path, cwd, ['pcb', 'export', 'stats'])) {
      throw new Error('KICAD_CAPABILITY_UNAVAILABLE: this KiCad CLI does not provide `pcb export stats`; upgrade KiCad or use DRC/ERC/BOM diagnostics instead.');
    }
  ...
  async diagnostics(workspace: string, projectPath: string, files: KicadProjectFiles) {
    ...
    return {
      ...
      ...(!provider.capabilities.boardStats && files.board ? { warnings: ['KiCad board statistics are unavailable in the detected CLI; diagnostics continue without stats.'] } : {})
    };
  }
