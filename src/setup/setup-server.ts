    ...
    try {
      ...
      if (url.pathname === '/api/execution-policy' && req.method === 'POST') {
        const body = await readJsonBody(req) as {
          ...
          workerRoutingProfile?: 'direct' | 'codex-assisted' | 'smart' | 'custom';
        ...
        const preset = requestedProfile === 'direct'
          ? { workerRoutingProfile: requestedProfile, defaultMode: 'rwmcp-only' as const }
          : requestedProfile === 'codex-assisted'
            ? { workerRoutingProfile: requestedProfile, codexEnabled: true, defaultMode: 'both' as const }
