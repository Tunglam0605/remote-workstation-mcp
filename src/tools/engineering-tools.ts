...
export function registerEngineeringTools(server: McpServer, ctx: AppContext): void {
  ...
  const flashSchema = z.object({
    ...
    probeSerial: z.string().optional(), targetConfig: z.string().optional()
  });
  ...
  server.registerTool('firmware_verify', {
    ...
    inputSchema: z.object({ workspace: z.string(), projectPath: z.string().default('.'), artifact: z.string().min(1), probeSerial: z.string().optional(), targetConfig: z.string().optional() }),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false }
  ...
  server.registerTool('target_reset', {
    ...
    inputSchema: z.object({ workspace: z.string(), projectPath: z.string().default('.'), probeSerial: z.string().optional(), targetConfig: z.string().optional() }),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }
  ...
  server.registerTool('debug_session_start', {
    ...
    inputSchema: z.object({ workspace: z.string(), projectPath: z.string().default('.'), symbols: z.string().min(1), probeSerial: z.string().min(1), targetConfig: z.string().optional() }),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }
