...
export function registerEngineeringTools(server: McpServer, ctx: AppContext): void {
  ...
  server.registerTool('engineering_profile_init', {
    ...
    inputSchema: profileProject.extend({
      ...
      firmware: z.object({
        buildProvider: z.enum(['auto', 'esp-idf', 'cmake', 'make']).optional(),
  ...
  server.registerTool('engineering_workflow_list', {
    description: 'List high-level typed engineering workflows available for a project from auto-detection and its canonical project profile.',
    inputSchema: profileProject,
    annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false }
  ...
  server.registerTool('engineering_workflow_plan', {
    description: 'Resolve a high-level engineering workflow into typed steps and concrete project defaults without executing build, flash, serial or ROS 2 commands.',
    inputSchema: profileProject.extend({ workflow: workflowId, overrides: workflowOverrides }),
    annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false }
  }, async ({ workspace, projectPath, workflow, overrides }) => result(await audited(ctx.audit, 'engineering_workflow_plan', workspace, () => ctx.engineering.workflows.plan(workspace, projectPath, workflow, overrides))));

  server.registerTool('engineering_workflow_run', {
    description: 'Run one approved high-level engineering workflow. Backend commands are generated by typed RWMCP adapters; arbitrary shell recipes are not accepted.',
    inputSchema: profileProject.extend({ workflow: workflowId, overrides: workflowOverrides }),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }
  }, async ({ workspace, projectPath, workflow, overrides }) => result(await audited(ctx.audit, 'engineering_workflow_run', workspace, () => ctx.engineering.workflows.run(workspace, projectPath, workflow, overrides))));
  ...
  server.registerTool('firmware_provider_status', {
    ...
    inputSchema: z.object({ provider: z.enum(['openocd']).default('openocd') }),
    annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false }
  ...
  server.registerTool('firmware_build', {
    description: 'Build a detected firmware project using a typed ESP-IDF, CMake or Make provider. Backend argv is generated by RWMCP, not supplied as a shell command.',
    inputSchema: z.object({ workspace: z.string(), projectPath: z.string().default('.'), provider: z.enum(['auto', 'esp-idf', 'cmake', 'make']).default('auto'), buildDir: z.string().default('build') }),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: false }
  }, async ({ workspace, projectPath, provider, buildDir }) => result(await audited(ctx.audit, 'firmware_build', workspace, () => ctx.engineering.firmware.build(workspace, projectPath, provider, buildDir))));
  ...
  server.registerTool('firmware_flash_plan', {
    ...
    annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false }
  }, async args => result(await audited(ctx.audit, 'firmware_flash_plan', args.workspace, () => ctx.engineering.firmware.flashPlan(args))));
  ...
  server.registerTool('firmware_flash', {
    ...
    inputSchema: flashSchema,
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }
  }, async args => result(await audited(ctx.audit, 'firmware_flash', args.workspace, () => ctx.engineering.firmware.flash(args))));
  ...
  server.registerTool('firmware_verify', {
    ...
    inputSchema: z.object({ workspace: z.string(), projectPath: z.string().default('.'), artifact: z.string().min(1), probeSerial: z.string().optional(), targetConfig: z.string().optional(), adapterSpeedKhz: z.number().int().min(50).max(24000).optional() }),
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false }
  }, async args => result(await audited(ctx.audit, 'firmware_verify', args.workspace, () => ctx.engineering.firmware.verify(args))));
  ...
  server.registerTool('target_reset', {
    ...
    inputSchema: z.object({ workspace: z.string(), projectPath: z.string().default('.'), probeSerial: z.string().optional(), targetConfig: z.string().optional(), adapterSpeedKhz: z.number().int().min(50).max(24000).optional() }),
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false }
  }, async args => result(await audited(ctx.audit, 'target_reset', args.workspace, () => ctx.engineering.firmware.reset(args))));
