...
test('v0.34.0 adds read-only worker routing while retaining Office Word and bounded worker authority on Action Schema v14 and Engineering API v5', async () => {
  ...
  assert.match(capabilities, /export const SERVER_VERSION = '0\.34\.0';/);
  ...
  const routeEnd = coreTools.indexOf("server.registerTool('work_objective_create'", routeStart);
  ...
  assert.doesNotMatch(routeBlock, /work_objective_execute_task|taskExecutor|dispatch\(/);
});
